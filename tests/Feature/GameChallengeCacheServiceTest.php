...
class GameChallengeCacheServiceTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function service(): GameChallengeCacheService
    {
        return app(GameChallengeCacheService::class);
    }

    protected function getFor(Game $game): GameChallenge
    {
        return $this->service()->getRecordByGameAndDate(CacheHelper::KEY_PLAY_GAME_PAGE, $game);
    }

    public function test_first_request_generates_saves_and_returns_the_game_challenge(): void
    {
        $this->travelTo(Carbon::parse('2026-09-01 09:00:00'));

        $game = $this->createZipGame();

        $gameChallenge = $this->getFor($game);

        $this->assertSame(1, GameChallenge::count());
        $this->assertSame($game->id, $gameChallenge->game_id);
        $this->assertSame('2026-09-01', $gameChallenge->game_date->toDateString());
        $this->assertNotNull($gameChallenge->board);
        $this->assertNotNull($gameChallenge->starts_at);
        $this->assertNotNull($gameChallenge->ends_at);
        $this->assertTrue($gameChallenge->relationLoaded('game'));
        $this->assertTrue($gameChallenge->relationLoaded('gameDifficulty'));
        $this->assertTrue($gameChallenge->game->relationLoaded('logo'));
    }

    public function test_second_request_returns_the_same_game_challenge_without_regenerating(): void
    {
        $this->travelTo(Carbon::parse('2026-09-01 09:00:00'));
    ...
    public function test_cache_hit_returns_the_same_game_challenge_without_a_database_query(): void
    {
        Cache::flush();
    ...
    public function test_cache_miss_with_existing_game_challenge_returns_the_database_record_without_a_new_board(): void
    {
        Cache::flush();
        config(['cache.cache_enable' => true]);
        ...
        $cacheKey = CacheHelper::cacheKeyGenerateSingleGameChallengeRecordByGameAndDate(
            CacheHelper::KEY_PLAY_GAME_PAGE,
            CacheHelper::KEY_GAME_CHALLENGE,
            $game,
            '2026-09-01',
        );
        ...
        CacheServerHelper::clearCached($cacheKey, [
            CacheHelper::KEY_PLAY_GAME_PAGE,
            CacheHelper::TAG_GAME_CHALLENGE,
        ]);
    ...
    public function test_new_day_creates_a_new_game_challenge_while_the_same_day_keeps_its_own(): void
    {
        ...
        $this->assertSame(2, GameChallenge::count());
    }

    public function test_difficulty_stays_stable_for_the_game_challenge(): void
    {
        $this->travelTo(Carbon::parse('2026-09-01 09:00:00'));
